    ...
    let description = this.state.editable ? <input type='text' ref={input => this.description = input} defaultValue={this.props.fruit.description}/>:<p>{this.props.fruit.description}</p>
        return(
            <div>
                {name}
                {description}
                <button onClick={() => this.handleEdit()}>{this.state.editable? 'Подтвердить' : 'Изменить'}</button>
                <button onClick={() => this.props.handleDelete(this.props.fruit.id)}> Удалить</button>
            </div>
